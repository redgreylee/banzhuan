...
class ExchangeHandler:
    ...
    def parse(self, raw_msg):
        ...
        try:
            ...
            if self.name == "Binance" and 'u' in data: 
                ...
                for i in items: self._update_and_check(i['symbol'], i['bid_px'], i['bid_sz'], i['ask_px'], i['ask_sz'])
                return
            
            if s: self._update_and_check(s, bp, bq, ap, aq)
        except: pass
    ...
    def _update_and_check(self, ex_s, bp, bq, ap, aq):
        ...
        if norm and bp and ap:
            ...
            prices_pool[norm][self.name] = (float(bp), float(bq), float(ap), float(aq), now)
    ...
    def _scan_symbol_live(self, sym, now):
        ...
        if best_buy_ex == best_sell_ex: return

        # 核心数据提取
        buy_p, buy_q = valid[best_buy_ex][2], valid[best_buy_ex][3] # 买入端价格与深度
        sell_p, sell_q = valid[best_sell_ex][0], valid[best_sell_ex][1] # 卖出端价格与深度
        ...
        if net_profit >= MIN_NET_PROFIT:
            arbitrage_opportunities.appendleft({
                ...
                "buy_ex": best_buy_ex, "buy_price": buy_p, "buy_val": round(buy_p * buy_q, 1),
                "sell_ex": best_sell_ex, "sell_price": sell_p, "sell_val": round(sell_p * sell_q, 1)
            })
